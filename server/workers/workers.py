...
def worker_onnx_audio_multiaccent(sentence, speaker_id, model_path, unique_model, vocoder_path, use_aliases,
                                  new_speaker_ids, use_cuda, temperature, speaking_rate):
    ...
    accent = ""
    input_speaker_id = 10000

    print(speaker_id)

    if use_aliases:
        for acc in spk_id_accents_dict.keys():
            print(acc)
            if speaker_id in spk_id_accents_dict[acc].keys():
                input_speaker_id = spk_id_accents_dict[acc][speaker_id]
                print(input_speaker_id)
                accent = acc
    else:
        input_speaker_id = speaker_id
    ...
    x = torch.tensor(
        intersperse(text_to_sequence(sentence, [cleaners[accent]]), 0),
        dtype=torch.long,
        device="cpu",
    )[None]
